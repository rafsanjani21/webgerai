      ...
      export default function Register() {
        ...
        const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>, setImgState: React.Dispatch<React.SetStateAction<string | null>>) => {
    const file = e.target.files?.[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        setImgState(reader.result as string); // Menyimpan format base64
      };
  ...
  <div>
    ...
    <p className="text-xs text-slate-500">(tekan foto untuk mengganti foto)</p>
    {selfieImg ? (
      <div className="relative group rounded-2xl overflow-hidden border border-slate-200 bg-slate-50">
        ...
        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
          <button type="button" onClick={() => setSelfieImg(null)} className="bg-rose-500 text-white px-4 py-2 rounded-xl font-bold text-sm shadow-sm hover:bg-rose-600 transition-colors flex items-center gap-2">
            🗑️ Ganti Foto
          </button>
        </div>
      </div>
    ) : (
      <div className="border-2 border-dashed border-slate-300 rounded-2xl p-5 text-center bg-slate-50 relative h-48 flex flex-col justify-center items-center gap-3">
        <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-sm text-lg">🤳</div>
        <p className="text-xs font-bold text-slate-500">Pilih metode pengambilan</p>
        <div className="flex gap-2 w-full justify-center">
          <button type="button" onClick={() => startCamera("selfie")} className="flex-1 max-w-[110px] bg-white border border-slate-200 text-slate-700 py-2 rounded-xl flex flex-col items-center gap-1 hover:border-blue-400 hover:text-blue-600 transition-all font-bold text-[11px] shadow-sm">
            <span className="text-base">📷</span> Kamera
          </button>
          <label className="flex-1 max-w-[110px] bg-white border border-slate-200 text-slate-700 py-2 rounded-xl flex flex-col items-center gap-1 hover:border-blue-400 hover:text-blue-600 transition-all font-bold text-[11px] shadow-sm cursor-pointer">
            <span className="text-base">📁</span> Galeri
            <input type="file" accept="image/*" className="hidden" onChange={(e) => handleFileUpload(e, setSelfieImg)} />
          </label>
        </div>
      </div>
    )}
  ...
  <div>
    ...
    {ktpImg ? (
      <div className="relative group rounded-2xl overflow-hidden border border-slate-200 bg-slate-50 flex items-center justify-center">
        ...
        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
          <button type="button" onClick={() => setKtpImg(null)} className="bg-rose-500 text-white px-4 py-2 rounded-xl font-bold text-sm shadow-sm hover:bg-rose-600 transition-colors flex items-center gap-2">
            🗑️ Ganti Foto
          </button>
        </div>
      </div>
      ...
      <div className="border-2 border-dashed border-slate-300 rounded-2xl p-5 text-center bg-slate-50 relative h-48 flex flex-col justify-center items-center gap-3">
        ...
        <p className="text-xs font-bold text-slate-500">Pilih metode pengambilan</p>
        <div className="flex gap-2 w-full justify-center">
          <button type="button" onClick={() => startCamera("ktp")} className="flex-1 max-w-[110px] bg-white border border-slate-200 text-slate-700 py-2 rounded-xl flex flex-col items-center gap-1 hover:border-blue-400 hover:text-blue-600 transition-all font-bold text-[11px] shadow-sm">
            <span className="text-base">📷</span> Kamera
          </button>
          <label className="flex-1 max-w-[110px] bg-white border border-slate-200 text-slate-700 py-2 rounded-xl flex flex-col items-center gap-1 hover:border-blue-400 hover:text-blue-600 transition-all font-bold text-[11px] shadow-sm cursor-pointer">
            <span className="text-base">📁</span> Galeri
            <input type="file" accept="image/*" className="hidden" onChange={(e) => handleFileUpload(e, setKtpImg)} />
          </label>
        </div>
      </div>
    )}
